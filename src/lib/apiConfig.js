...
export async function getIsSaudiFromHeaders(headersObj) {
	...
	// 			console.log(`[Geo] Resolved via header '${h}': ${countryCode} (isSaudi: ${isSaudi})`);
	...
	// 		console.log(`[Geo] Querying IPInfo for IP: ${ipUsed}`);
	...
	// 			console.log(`[Geo] IPInfo result: ${countryCode} (isSaudi: ${isSaudi})`);
	...
	// 		countryCode = "SA";
	// 		isSaudi = true;
	// 		console.log(`[Geo] Fallback to Accept-Language 'ar': isSaudi=true`);
	// 		return { isSaudi, countryCode, ipUsed };
	// 	}
	// } catch (e) {
	// 	// swallow errors, keep defaults
	// 	console.error("[Geo] Error during geolocation:", e);
	// }
		
	// // default: assume Saudi to keep prices in SAR unless we know otherwise
	// console.log(`[Geo] Default fallback: isSaudi=${isSaudi}`);
	return { isSaudi, countryCode, ipUsed };
}

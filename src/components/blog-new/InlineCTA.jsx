        ...
        <div className="overflow-hidden rounded-3xl bg-gradient-to-br from-[#E7D3AF] to-[#e8dcc4] shadow-lg">
          <div className="px-8 py-10 text-center md:px-12">
            ...
            <p className="mb-8 text-xl leading-relaxed text-[#4a5568]">
              {isAr ? "من القراءة إلى التجربة الحية — اختر جولة مصممة بعناية" : "From reading to live experience — choose a carefully designed tour"}
            </p>

            {/* Buttons */}
            <div className="flex flex-col justify-center gap-4 sm:flex-row">
              <button className="group flex items-center justify-center gap-3 rounded-full bg-[#3C6652] px-8 py-4 text-xl text-white transition-all hover:bg-[#2d4d3d] hover:shadow-xl">
                <span >{isAr ? "استكشف الجولات المرتبطة" : "Explore Related Tours"}</span>
                <Compass className="h-5 w-5 transition-transform group-hover:rotate-45" />
              </button>
              <button className="flex items-center justify-center rounded-full border-2 border-[#3C6652] bg-transparent px-8 py-4 text-xl text-[#3C6652] transition-all hover:bg-[#3C6652] hover:text-white hover:shadow-xl">
                <span>{isAr ? "تصفح جميع الرحلات" : "Browse All Trips"}</span>
              </button>
            </div>
          </div>
        </div>

...
export default function Offer({ offerEndDate }) {
	...
	// const end = isEven(today.getDate())
	// 	? today.getDate() + 1
	// 	: today.getDate() + 2;
	// const formattedEnd = String(end).padStart(2, "0"); // Ensure double-digit format for day
  // comment out because we not using it for now 
	// useEffect(() => {
	// 	if (typeof window !== "undefined") {
	// 		setLanguage(localStorage.getItem("lang") || "en");
	// 	}

	// 	const openingDate = offerEndDate
	// 		? new Date(offerEndDate.replace(" ", "T") + "Z").getTime()
	// 		: new Date(`${year}-${month}-${formattedEnd}T00:00:00Z`).getTime();

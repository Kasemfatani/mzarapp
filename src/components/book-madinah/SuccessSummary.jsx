...
export default function SuccessSummary({ initialLang = "en" }) {
	...
	useEffect(() => {
		(async () => {
			...
			// Persist tran id for UI (keep existing state if present)
			...
			if (status === "pending") {
				// small delay for server callback to arrive
				...
				try {
					...
					setTranId(tranRef);
					// update url to success state so UI shows success
					const newUrl = new URL(window.location.href);
					newUrl.searchParams.set("status", "success");
					newUrl.searchParams.set("tranRef", tranRef);
					window.history.replaceState({}, "", newUrl.toString());
					return; // will re-run effect because tranId changed
				} catch (err) {
			...
			console.log("Finalizing madinah booking with payload:", payload);
			...
			try {
				...
				console.error("madinah-booking-create error", e);
				setFinalizeError(true);
				setSubmitting(false);
			}
		})();

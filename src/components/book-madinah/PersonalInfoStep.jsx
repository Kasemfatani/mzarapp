...
export default function PersonalInfoStep({
	...
	// Promo code state (same behavior as book-haram version)
	...
	// Apply promo code: GET check endpoint
	const handleApplyPromo = async () => {
		...
		try {
			...
			// check json.status even if HTTP 200
	...
	const onSubmit = async (values) => {
		...
		// Build payload for booking API (include promo_code if applied)
		const payload = {
			name: info.name,
			email: info.email,
		...
		try {
			...
			// Calculate final total for URWAY payment (apply discount then tax)
			...
			const urwayRes = await fetch("/api/pay/urway/init", {
				...
				body: JSON.stringify({
					...
					lang,
					flow: "bus",
					successPath: "/book-madinah-success",
					failPath: "/book-madinah",
				}),
			});
	...
	// Calculation logic (updated to include promo discount)

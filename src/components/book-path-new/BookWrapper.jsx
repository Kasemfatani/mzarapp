...
import { getAddonUnitPrice } from "@/lib/addonPricing"; // <-- NEW import
...
const getSchema = (lang, max_people_count = 20, min_people_count = 1) => {
	...
	const reqVehicle = lang === "ar" ? "المركبة مطلوبة" : "Vehicle is required";
		...
		form.handleSubmit(
			async (values) => {
				try {
					...
					if (method === "online") {
						// 1. Calculate 5% discount on the Raw Base
						const onlineDiscount = Number((rawBase * 0.05).toFixed(2));

						// 2. New Taxable Base = (RawBase + Addons) - PromoDiscount - OnlineDiscount
						const onlineTaxable = Number(
							(rawBase + addonsTotal - discountAmount - onlineDiscount).toFixed(
								2,
							),
						);
